...
node* binary_st::search_bst(int key){
    node* itr = root;
    while(itr){
        if(itr->data<key){
            itr = itr->right_c;
...
int main()
{
    ...
    int num;
    cout<<"Enter the number you want ot find in the Binary Search Tree : ";
    cin>>num;

    if(b1.search_bst(num)==nullptr) cout<<"Element is not present in the tree.";
    else if(num == b1.search_bst(num)->data){
